...
- **Review telemetry (`jewzaam-reviews`)** — the orchestrator stamps four resource attributes on each child agent: `review_run_id`, `review_stage` (`select`/`review`/`validate`), `review_agent`, `review_scoring`.
  - **Children carry no `host_name`, `project`, `sandbox_source`, `sandbox_profile` or `user_name`.** `orchestrator/backend.py:_scrubbed_env()` copies `OTEL_RESOURCE_ATTRIBUTES` out of its own environment, but Claude Code does not export that variable into tool subprocesses, so there is nothing to copy. Review streams expose 58 structured-metadata fields against 79 on a normal session.
