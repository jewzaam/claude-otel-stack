...
- **Loki table panels need `"format": "table"` + `"instant": true`** on every target. A target left as a range query (`"range": true`) feeds time-series frames into `joinByField`, and the panel renders **No Data** with no error — the queries are fine and return rows against `/loki/api/v1/query`. Both existing table panels use the instant form; copy it.
...
- **Review telemetry (`jewzaam-reviews`)** — the orchestrator stamps four resource attributes on each child agent: `review_run_id`, `review_stage` (`select`/`review`/`validate`), `review_agent`, `review_scoring`.
  - **Children now carry `host_name`, `project`, `sandbox_source` and `sandbox_profile`; only `user_name` is still missing.** Verified live on run `ae262694383a` (2026-08-27). This reverses the earlier finding that `orchestrator/backend.py:_scrubbed_env()` had no `OTEL_RESOURCE_ATTRIBUTES` to copy — check the data before relying on either claim. The session-state rules depend on it: they fold children into the launching session and sum parent and child events into one series, which only works while the labels match.
  - **`trace_id` is the join to identity.** Every review event carries one (verified 6240/6240 over 30d) and each run has exactly one distinct trace, shared with the parent Claude Code session. `review_run_id` → `trace_id` → parent events recovers host, project, `sandbox_source` and the parent `session_id`.
  - **That join is many-to-one.** Two runs launched from the same parent interaction share a `trace_id` — observed live. Traverse run → trace → identity; the reverse direction silently merges the two runs into one row.
  - **Prefilter the identity side with `and on (trace_id)`.** An unrestricted `sum by (trace_id, ...)` over parent events hits Loki's 500-series cap (507 distinct traces in 30d). Restricting to traces that have review children reduces it to one series per run.
...
|--------|---------|-------------|
| `claude_session_working` | Newest event is activity, not Stop (10m window) | `api_request`, `tool_decision`, `tool_result`, `skill_activated`, `subagent_completed`, `user_prompt` timestamp > Stop timestamp |
...
- **`count_over_time` needs `sum by` wrapper** — `count_over_time` does not support `by()` grouping directly. `max_over_time` does, via its trailing `) by (...)`, which is why the state rules use it.
- **Prometheus remote write receiver** must be enabled via `--web.enable-remote-write-receiver` flag on the Prometheus container.
- **`user_prompt` must be in the WORKING rule's event list.** It is what ends READY (`claude_session_ready` compares Stop against activity, and `user_prompt` is activity), and under the timestamp-comparison WORKING rule it is also what holds a session working through a turn that emits nothing else. Leave it out and a session prompted after >60s idle falls into *no* state metric until its first `api_request` — measured against live Loki, median 21s and up to 80s — and `claude-dashboard` drops the remote row for that window.
- **WORKING is a timestamp comparison, not an activity count.** It was `count_over_time(...[60s])` and went false during any turn that emitted nothing for 60 seconds. Server-side tools make that routine: the advisor (`/advisor`) executes on Anthropic's side, arrives as a `server_tool_use` block named `advisor`, returns an `advisor_tool_result` block, and runs no local tool — so no `tool_decision`, no `tool_result`, and no hook fires. The whole consultation sits inside one in-flight `api_request`, which emits only on completion. Verified by reading the Claude Code bundle at 2.1.269 (`bin/claude.exe` is a Bun single-file executable; the JS is embedded as plain text). There is no client-side event to count, so no event-list change fixes it — only the ordering comparison does. Long tool calls and long background agents had the same hole. Accepted costs: the metric's value is now an event timestamp, and a session that dies mid-turn without firing Stop reads WORKING until its activity leaves the window. **`StopFailure` is not treated as end-of-turn** in either WORKING or READY, so an errored turn holds WORKING for the rest of the window — fixing that means adding it to both rules, not one.
- **WORKING's window is `[10m]`, READY's and PERMISSION's are `[30m]`. Deliberate, not drift.** They do different jobs: READY's window is what keeps a finished session on the dashboard, WORKING's is how long a silent turn stays visible. In WORKING only the activity side binds — a Stop falling out of the window hits the `or (0 * activity)` pad and loses to any activity timestamp, which is safe because idle means Stop is newer than the last activity, so activity in the window implies Stop in the window. The window therefore means exactly one thing: the longest OTEL-silent stretch inside a turn that stays visible. A thinking session and a dead one both emit nothing, so that same number is how long a dead session lies about working. One knob, both meanings, and no out-of-band liveness signal exists to split them — `claude_code.llm_request` is a span and only exports on completion.
- **The 10m came from measurement.** Over 5535 intra-turn gaps: p50 0.1s, p90 6.0s, p99 57.6s. The previous `[60s]` count window sat exactly on the p99, so ~1 gap in 100 blanked the row. Share of gaps exceeding each candidate: 60s 0.94%, 300s 0.23%, 600s 0.13%, 1800s 0.05%. The errors are asymmetric — too small drops a live row briefly and self-corrects on the next event (`REMOTE_METRIC_GRACE_SECONDS=120` absorbs it), too large leaves a dead session claiming WORKING until dismissed by hand — so the knee, not the tail, is the right target. Re-derive with `make measure-turn-gaps DAYS=7` before changing it; `turns_lost_by_window.working_only` excludes time parked at a permission dialog or AskUserQuestion and turns the user ended by typing again, so it can only argue the window down. The floor is one main-thread `api_request`, not a whole turn — agent `api_request`s carry `query_source=agent:*` and are not filtered out, so agent-heavy sessions are never silent.
...
- **Orchestrated children are folded into their launching session** — events carrying `review_orchestrating_session_id` (jewzaam-reviews `review` skill, headless `claude -p` agents) get `session_id` rewritten to that value in WORKING and READY, and are dropped from PERMISSION. Otherwise the launching session, which backgrounds the run and stops, reports READY for the entire 15-50 minute review while its agents spend. **Fold in both rules or neither.** With WORKING alone, each child still published its own `claude_session_ready` on its first Stop — carrying the parent's sandbox labels, since children inherit the parent's environment — and claude-dashboard maps every such series onto the same sandbox row, last one wins, so a finished child's READY overwrote the live parent's WORKING. Observed live on run `ae262694383a`. PERMISSION drops rather than folds: nobody can answer a headless agent's prompt, and the dashboard treats PERMISSION_REQUIRED as sticky until dismissed by hand. Children keep full sandbox labels, so parent and child events sum into one series with no duplicate label set — verified live.
